import { ComponentDialog, WaterfallDialog, Dialog, DialogTurnResult, DialogSet, DialogTurnStatus } from "botbuilder-dialogs";
...
export class SsoDialog extends ComponentDialog {
    ...
    constructor(dedupStorage: Storage, requiredScopes: string[]) {
        ...
        const ssoDialog = new TeamsBotSsoPrompt(teamsFx,
            TEAMS_SSO_PROMPT_ID,
            {
    ...
    public addCommand(commandId: string, commandText: string | RegExp, operation: (context: TurnContext, ssoToken: string) => Promise<DialogTurnResult>): void {
    ...
    private matchCommands(text: string): string {
        for (const command of this.commandMapping) {
            let pattern: string | RegExp = command[1];
